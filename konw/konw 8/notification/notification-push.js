...
function appStart() {
    // POWIADOMIENIA
    pokazPowiadomienie()
    // niektóre przeglądarki wymagają requestPermission jako efektu akcji użytkownika
    document.querySelector('#user-notify').addEventListener('click', pokazPowiadomienie)
}

async function pokazPowiadomienie() {
    // 1. Czy domena po zezwolenie?
    // 2. w Incognito nie ma powiadomień
...
function showNotify(title, body) {
    ...
    notify.addEventListener('close', notifyClose) // zamknięcie krzyżykiem przez użytkownika
}
